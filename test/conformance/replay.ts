...
function simulation(action: string, prior: k.TicketGraph): k.TicketCommand {
  switch (action) {
    case "createOne":
...
function event_command(e: k.TicketEvent): Reconstructed {
...
export function command_from_step(
  step: DecisionStep,
  prior: k.TicketGraph,
): Reconstruction {
  if (step.action !== null) {
